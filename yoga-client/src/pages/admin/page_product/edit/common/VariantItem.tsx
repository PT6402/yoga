...
interface Props {
  ...
  handleRemoveItem: ({ variantId }: { variantId: number }) => void;
}
export default function VariantItem({
  ...
  const handleAddVariantItem = () => {
    if (
      ...
      if (valueSelect && item.id && valueSelect.id && valueSelect.value) {
        setVariantItems((prev) => [
          ...prev,
          {
            value: valueSelect.value!,
            price_modifier: valuePrice,
            id: valueSelect.id!,
          },
        ]);
        handleAddItem({
          typeId: item.id,
          variantId: valueSelect.id,
          price_modifier: valuePrice,
        });
  ...
  const handleRemoveVariantItem = (itemId?: number, value?: string) => {
    if (itemId && item.id && value) {
      const variantId = item.variants.find((p) => p.value == value);
      const newList = variantItems.filter((pi) => pi.value != value);
      if (variantId) {
        handleRemoveItem({ variantId: variantId.id });
        ...
        <div className="p-6 flex flex-col sm:flex-row items-center">
          ...
          >
            <h3 className=" text-slate-700 dark:text-slate-300 flex ">
              <Chip variant="ghost" value={item.name} />
            </h3>
          ...
          <ButtonSecondary
            ...
            onClick={() => handleRemoveItemType(item.id)}
          >
            remove
          </ButtonSecondary>
        </div>
                          ...
                          variantItems.map((vi) => (
                            <tr key={vi.id}>
                              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                                {vi.value?.toUpperCase()}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {formatPrice(vi.price_modifier)}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                <Button
                                  ...
                                  color="red"
                                  onClick={() =>
                                    handleRemoveVariantItem(vi.id, vi.value)
                                  }
                                >
